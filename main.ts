...
namespace Blur {
    ...
    export function NewSetBlurFilter(size: number) {
        ...
        let num1 = 0
        precalc = []
        for (let index3 = 0; index3 < 119 + size / 2; index3++) {
            precalc.push(Math.constrain((Math.round(index3 / size) * size) + size / 2, 0, 119 + size / 2))
        }
        let variable = scene.createRenderable(zLayer, (image: Image, camera: scene.Camera) => {
            for (let index = 0; index < 159; index++) {
                savedx = Math.constrain((Math.round(index / size) * size) + size / 2, 0, 159 + size / 2)
                for (let index2 = 0; index2 < 119; index2++) {
                        if (!(precalc[index2] == index2)) {
